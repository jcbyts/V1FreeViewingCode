...
for sessionId =8
    [Exp,S] = io.dataFactory(sessionId, dataPath);
end


%%

% addpath ~/Dropbox/Projects/dDPI/code/
...
plot(xf)
%%
fixation = Exp.vpx.Labels == 1;
spd = Exp.vpx.smo(:,7);
fixation= spd < 2;

ix = 1:10e5;
x = x(ix);
fixation = fixation(ix);
sum(fixation)
[bw, num] = bwlabel(fixation);
inds = cell(num,1);
for i = 1:num
    inds{i} = find(bw==i);
    inds{i} = inds{i}(2:end-1);
end
inds(cellfun(@numel, inds) < 20) = [];
% 
% numel(x)
...
lasti = 0;
xc = zeros(sum(cellfun(@numel, inds)), 1);
for i = 1:num-1
    xtmp = x(inds{i});
    xtmp = xtmp - mean(xtmp(find(~isnan(xtmp),2)));
    
    iix = inds{i}-inds{i}(1) + 1;
    if i == 1
        xc(iix) =  xtmp;
    else
        xlast = xc(inds{i-1});
        xc(iix + lasti) =  xtmp + mean(xlast(find(~isnan(xlast), 2, 'last')));
        lasti = iix(end) + lasti;
    end
%     plot(inds{i}, x(inds{i}), 'r'); 
end

%%
% depochs = detectDrift(x)
% xc = concatenateSignal(x, inds);
xc(isnan(xc)) = [];
xc = xc(1:10e3);
figure(2); clf
plot(xc)
...
figure(1); clf
pwelch(xc- nanmean(xc), [], [], [], Fs)

%%
validTrials = io.getValidTrials(Exp, 'Grating');

validTrials = validTrials(1);
[Stim, frameInfo] = regenerateStimulus(Exp, validTrials, S.rect, 'includeProbe', false);
...
figure(1); clf
nFrames = size(Stim,3);
h = imagesc(Stim(:,:,1)); colorbar
for i = 2:nFrames
    h.CData = Stim(:,:,i)-Stim(:,:,i-1);
    colorbar
    drawnow
    pause
end
...
X = reshape(Stim, prod(frameInfo.dims), numel(frameTimes))';
[Y, binfun] = binNeuronSpikeTimesFast(Exp.osp, frameTimes, spikeBinSize);
