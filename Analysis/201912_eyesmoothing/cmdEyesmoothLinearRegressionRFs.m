...
fitDir = 'C:\Users\Jake\Dropbox\Projects\FreeViewing\Output';

%% regenerate data with new parameters?
for eyeSmooth = [1 3 5 9 21]
    options = {'stimulus', 'Gabor', ...
        'testmode', false, ...
        'eyesmooth', eyeSmooth, ... % bins
        't_downsample', 2, ...
        's_downsample', 2, ...
        'includeProbe', true, ...
        'correctEyePos', true};
    ...
    for inl = 1:2
        ...
        xy = xy(1:end-1,:);
        d = size(X,2);
        stas = (C + lambda*eye(d))\xy;
        
        fsave = fullfile(fitDir, sprintf('%s_esm_%d_%s.mat', strrep(fname, '.mat', ''), eyeSmooth, inonlin{inl}));
        save(fsave, 'stas')
    end
...
eyeSmooth = 21;
inonlin = 'raw';
fsave = fullfile(fitDir, sprintf('%s_esm_%d_%s.mat', strrep(fname, '.mat', ''), eyeSmooth, inonlin));
tmp = load(fsave);
stas = tmp.stas(:,cids);
N = size(stas,2);
figure(eyeSmooth)
for cc = 1:N
    ...
    for i = 1:nlags
        subplot(N,nlags,(cc-1)*nlags + i, 'align')
...
%     inds = unique(slist(:,2) + saclags(i) + (0:2);
    ...
    xy = [f(X(inds,:)) ones(numel(inds),1)]'*Robs(inds,cids);
