...
eyeX = deepcopy(eyeAtFrameCentered[rinds,0])
eyeY = deepcopy(eyeAtFrameCentered[rinds,1])
...
for ith in tqdm(range(nsteps)):
    ...
    # print("getting crop indices th: %d" %th)
    ...
    Xstim, rinds = ne.create_time_embedding_valid(stmp, [num_lags, NX, NY], valdata)
    ...
    for xx in range(Npos):
        for yy in range(Npos):
            ...
            if len(valtot) > 1000: # at least 100 samples to evaluate a likelihood
