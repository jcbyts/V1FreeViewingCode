...
class LBFGS(Optimizer):
    ...
    def _add_update(self, step_size, update):
        ...
        for p in self._params:
            ...
            # TODO: this is now deprecated... what to do?
            p.data = p.data + (update[offset:offset + numel].view_as(p.data) * step_size)
    ...
    def two_loop_recursion(self, vec):
        ...
        for i in range(num_old - 1, -1, -1):
            ...
            q = q + (-alpha[i] * old_stps[i]) # attempting to avoid deprecation warning
            # q.add_(-alpha[i], old_stps[i]) # old deprecated way
        ...
        for i in range(num_old):
            ...
            # r = r + (alpha[i] - beta) * old_dirs[i] # how do you do inplace add
            # r.add_(alpha[i] - beta, old_dirs[i]) # avoid deprecation warning
