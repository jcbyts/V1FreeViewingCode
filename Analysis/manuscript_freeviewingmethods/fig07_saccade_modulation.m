...
ax = nexttile(3, [2 2]);
relrateni = nimrate(ix,:) ./ median(nimrate(ix,:),2);
relrategr = grmrate(ix,:) ./ median(grmrate(ix,:),2);
...
% Null
iix = fftrf{iEx}.rfs_post(cc).xproj.bins <= fftrf{iEx}.rfs_post(cc).xproj.levels(1);
